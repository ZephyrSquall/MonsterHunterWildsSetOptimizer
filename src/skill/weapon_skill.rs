...
pub const WEAPON_SKILLS: [Skill; 12] = [
    Skill {
        name: "Airborne",
        alt_name: None,
        id: SkillId::Airborne,
        ...
        max: 1,
        modifier: Modifier {
            // Airborne modifies attack by increasing raw damage by 10% on jump attacks, so provide
            // a closure that describes this behaviour.
            attack: |level, bonus_attack, weapon| {
                *bonus_attack +=
                    f64::from(weapon.attack) * 0.1 * f64::from(level) * AIRBORNE_COVERAGE;
            },
    ...
    Skill {
        ...
        modifier: Modifier {
            ...
            // I don't find this a particularly reliable source since it doesn't seem to be
            // datamined info, but I am unable to find exact numbers elsewhere, so I'm trusting it
            // for now. This will be updated if I find corrected numbers or other sources that backs
            // this up.
    ...
    Skill {
        ...
        max: 3,
        modifier: Modifier {
            // I cannot find any numbers for Critical Status anywhere. Until I find hard numbers for
            // it, I'm ignoring it by returning nothing for the bonus status critical multiplier.
    ...
    Skill {
        ...
        skill_type: SkillType::Weapon,
        max: 1,
        // TODO: Find a way to implement Master's Touch. This is not a simple skill to implement, as
        // it depends on the final affinity value, which isn't known at the time these modifiers are
        // run. For now I'm simply ignoring it.
        modifier: Modifier { ..DEFAULT_MODIFIER },
    },
    Skill {
